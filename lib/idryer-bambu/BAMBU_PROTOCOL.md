...
| **iHeater-link (текущее)** | Существующий клиент в репо | `lib/idryer-core/src/integrations/bambu/bambu_client.cpp`, `BAMBU_MQTT_FIELDS.md` |
...
`lib/idryer-core/src/integrations/bambu/bambu_client.cpp` — берёт первый непустой
...
2. Управление подключением: сохраняем существующий `BambuClient` из `idryer-core`
   и патчим парсинг, или пишем заново? (формулировка задачи указывает
   на второй вариант)
