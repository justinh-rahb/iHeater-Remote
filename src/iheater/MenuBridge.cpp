...
constexpr size_t kConnectionTogglesCount = sizeof(kConnectionToggles) / sizeof(kConnectionToggles[0]);
...
int disableOtherConnections(const char* keepBind) {
    ...
    for (size_t i = 0; i < kConnectionTogglesCount; i++) {
        ...
        if (keepBind && strcmp(b, keepBind) == 0) continue;
        bool cur = false;
...
void normalizeExclusivity() {
    ...
    for (size_t i = 0; i < kConnectionTogglesCount; i++) {
        ...
        if (menu_read_by_bind(kConnectionToggles[i], &cur) && cur) {
            if (!firstOn) firstOn = kConnectionToggles[i];
            else {
...
void MenuBridge::begin() {
    ...
    if (!menu_nvs_begin()) {
        HAL_LOG_WARN("MENU", "NVS begin failed — работаем только с дефолтами в RAM");
    }
    ...
    //    выбранный источник (важно если в NVS уже было включено, скажем, moon_en).
...
ActiveConnection MenuBridge::currentActive() const {
    bool en = false;
    if (menu_read_by_bind("bambu_en", &en) && en) return ActiveConnection::Bambu;
    if (menu_read_by_bind("moon_en",  &en) && en) return ActiveConnection::Moonraker;
    if (menu_read_by_bind("ha_en",    &en) && en) return ActiveConnection::Ha;
    return ActiveConnection::None;
}

void MenuBridge::emitActiveIfChanged() {
    ActiveConnection cur = currentActive();
    if (cur == lastActive_) return;
    lastActive_ = cur;
    if (activeCb_) activeCb_(cur);
}
...
bool MenuBridge::applySetCommand(JsonObjectConst data) {
    ...
    // Два способа адресации: по id (как у iDryer) или по bind (удобнее для отладки).
    ...
    if (data["bind"].is<const char*>()) {
        ...
        for (uint16_t i = 0; i < g_bindings_count; i++) {
            if (g_bindings[i].id == (uint16_t)id) { b = &g_bindings[i]; break; }
        }
    }
    ...
    float val = 0.0f;
    if (data["val"].is<float>()) val = data["val"].as<float>();
    else if (data["val"].is<bool>()) val = data["val"].as<bool>() ? 1.0f : 0.0f;
    else if (data["val"].is<int>()) val = (float)data["val"].as<int>();
    else {
    ...
    HAL_LOG_INFO("MENU", "set: %s = %.3f (id=%u)", b->bind, (double)val, (unsigned)b->id);
    return true;
}
